...
def model_detection(imagen_path, calibracion_ancho=1, calibracion_altura=1, model_path='./best.pt', distancia_minima=50):
    # cargar el modelo
    ...
    for obj_idx, mascaras in enumerate(results[0].masks):
        ...
        if not descartar_objeto:
            ...
            obj  = {"altura_pix": altura_objeto, "ancho_pix": prom_sec_objeto, "altura_med":altura_calibrada ,"ancho_med": ancho_calibrado}
            resultados_objetos.append((ancho_calibrado, altura_calibrada))
            result_total.append(obj)
